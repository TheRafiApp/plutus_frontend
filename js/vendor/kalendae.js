...
var Kalendae = function (targetElement, options) {
	...
	// Custom class range addition
	if (typeof opts.rangeClass === 'function') {
		self.rangeClass = opts.rangeClass;
	} else {
		self.rangeClass = function () {return false;};
	}
...
Kalendae.prototype = {
	...
	directions: {
		...
		'select-future' :function (date) {return moment(date).startOf('day') < this.settings.selected[0];}
	},
	...
	addSelected : function (date, draw) {
		...
		switch (this.settings.mode) {
			...
			case 'range':
				...
				if (this.settings.rangeLock) if (this._sel.length > 1) this.loaded = true;

				if (this.settings.rangeLock && this.loaded) {
					if (this._sel[0] < date) this._sel[1] = date;
				} else {
	...
	draw : function draw() {
		...
		do {
			...
			do {
				...
				if (this.rangeClass(day)) klass.push(this.rangeClass(day));
		...
		if (opts.directionScrolling) {
			...
			} else if (opts.direction === 'today-future' || opts.direction === 'future' || opts.direction==='select-future') {
				if (diff > opts.months) {
...
Kalendae.Input = function (targetElement, options) {
	...
	opts.attachTo = $input.parentElement;
	...
	if (overwriteInput) {
		if (this.settings.mode === 'range' && this.settings.rangeLock) {
			$input.value = self.getSelectedRaw()[1].format('MM/DD/YYYY'); // hack for our app
		} else {
	...
	this._events.inputFocus = util.addEvent($input, 'focus', function () {
		...
		if (self.settings.mode === 'range' && self.settings.rangeLock) {
			var dates = parseDates(this.value, self.settings.parseSplitDelimiter, self.settings.format);
			if (dates.length > 1) {
				self.setSelected([self.getSelectedRaw()[0], dates[1]]);
			} else {
				self.setSelected([self.getSelectedRaw()[0], dates[0]]);
			}
	...
	this._events.inputKeyup = util.addEvent($input, 'keyup', function (event) {
		...
		if (dateValue && dateValue.length && dateValue[0] && dateValue[0].year() > 1000) {
			if (self.settings.mode === 'range' && self.settings.rangeLock) {
				self.setSelected([self.settings.selected[0], this.value]);
			} else {
				self.setSelected(this.value);
			}
		} else {
	...
	self.subscribe('change', function () {
		...
		if (this.settings.mode === 'range' && this.settings.rangeLock) {
			// make sure the range field has run once already
			if (self.getSelectedRaw().length > 1) {
				$input.value = self.getSelectedRaw()[1].format('MM/DD/YYYY'); // only show the end date
			} else {
				$input.value = self.getSelected();
			}
		} else {
...
Kalendae.Input.prototype = util.merge(Kalendae.prototype, {
	...
	show : function () {
		...
		util.addClassName($container, 'kalendae-input-open');
	...
	hide : function () {
		...
		util.removeClassName(this.container, 'kalendae-input-open');
